...
function AbogadosTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { abogados, error, isLoading, isValidating, refresh } = useAbogados(); // EDITABLE
    ...
    const [selectedAbogado, setSelectedAbogado] = useState(null); // Objeto seleccionado: EDITABLE

    // --------------- Funciones necesarias para persistencia ----------------------------------------

    useEffect(() => {
        // Retrieve persisted state from session storage
        const savedState = sessionStorage.getItem('abogados-table-state');
    ...
    const initFilters = () => {
        setFilters({
            ...
            identificacion: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de Abogados', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: abogados.map((row) => cols.map((column) => formatDateField(row[column.dataKey]))), // EDITABLE
                ...
                doc.save('Abogados.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Abogados'); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Abogados'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {abogados ? abogados.length : '0'}</span>
    ...
    const onRowSelect = (event) => {
        props.onSelect(event.data.abogadoId);
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="abogadoId" // EDITABLE
                filters={filters}
                globalFilterFields={['nombre', 'apellido', 'identificacion', 'matricula', 'email', 'telefono']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron abogados" // EDITABLE
                selectionMode="single"
                selection={selectedAbogado} // EDITABLE
                onSelectionChange={(e) => setSelectedAbogado(e.value)} // EDITABLE
                onRowSelect={onRowSelect}
                metaKeySelection={false}
                stateStorage="session" 
                stateKey="abogados-table-state" // EDITABLE
            >
