...
function ClasesTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { clases, error, isLoading, isValidating, refresh } = useClases(); // EDITABLE
    ...
    const [selectedClase, setSelectedClase] = useState(null); // Objeto seleccionado: EDITABLE

    // --------------- Funciones necesarias para persistencia ----------------------------------------

    useEffect(() => {
        // Retrieve persisted state from session storage
        const savedState = sessionStorage.getItem('clases-table-state');
    ...
    const cols = [
        ...
        { header: 'Descripción en español', dataKey: 'descripcionEspanol' },
        { header: 'Descripción en inglés', dataKey: 'descripcionIngles' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de Clases', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: clases.map((row) => cols.map((column) => formatDateField(row[column.dataKey]))), // EDITABLE
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Clases'); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Clases'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {clases ? clases.length : '0'}</span>
    ...
    const onRowSelect = (event) => {
        props.onSelect(event.data.codigo);
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                filters={filters}
                globalFilterFields={['codigo', 'descripcionEspanol', 'descripcionIngles']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron clases" // EDITABLE
                selectionMode="single"
                selection={selectedClase} // EDITABLE
                onSelectionChange={(e) => setSelectedClase(e.value)} // EDITABLE
                onRowSelect={onRowSelect}
                metaKeySelection={false}
                stateStorage="session" 
                stateKey="clases-table-state" // EDITABLE
            >
