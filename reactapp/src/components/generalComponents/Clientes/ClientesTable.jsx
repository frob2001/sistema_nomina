...
const API_BASE_URL = `${apiEndpoint}/Clientes/Buscar`;
// Auth
...
function ClientesTable(props) {
    ...
    const { data: clientesPre, error, isValidating, isLoading, mutate: refresh } = useSWR(searchURL, fetcher, {
        errorRetryInterval: 10000,
    });

    const clientes = useMemo(() => {
        if (clientesPre) {
            return clientesPre.map(cliente => ({
                ...cliente,
                paisNombre: cliente.pais?.nombre
            }));
        }
    }, [clientesPre]); // Añade el nombre del país como propiedad para que funcione bien con la datatable
    ...
    useEffect(() => {
        const savedState = sessionStorage.getItem('clientes-table-state'); // EDITABLE
    ...
    const initFilters = () => {
        setFilters({
            ...
            clienteId: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
    ...
    const cols = [
        { header: 'Código del Cliente', dataKey: 'clienteId' },
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de Clientes', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: clientes.map((row) => cols.map((column) => formatDateField(row[column.dataKey]))), // EDITABLE
                ...
                doc.save('Clientes.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Clientes'); // EDITABLE
        ...
        clientes.forEach((cliente) => { // EDITABLE
            worksheet.addRow([ 
                cliente.clienteId,
                cliente.nombre,
                cliente.ciudad,
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Clientes'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {clientes ? clientes.length : '0'}</span>
    ...
    const onRowSelect = (event) => {
        props.onSelect(event.data.clienteId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="clienteId" // EDITABLE
                filters={filters}
                globalFilterFields={['clienteId', 'nombre', 'ciudad', 'paisNombre', 'email', 'telefono']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron clientes" // EDITABLE
                ...
                stateKey="clientes-table-state" // EDITABLE
            >
