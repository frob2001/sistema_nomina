...
function GacetasTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { gacetas, error, isLoading, isValidating, refresh } = useGacetas(); // EDITABLE

    const formatDate = (dateString) => {
        const options = { year: 'numeric', month: 'long', day: 'numeric' }; // Spanish format
        return new Date(dateString).toLocaleDateString('es-ES', options);
    };

    const flatGacetas = useMemo(() => {
        if (gacetas) {
            return gacetas.map(gaceta => ({
                ...gaceta,
                paisNombre: gaceta.pais?.nombre,
                fecha: gaceta.fecha ? formatDate(gaceta.fecha) : null,
            }));
        }
    }, [gacetas]); // Prepara los datos para la tabla
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('gacetas-table-state');
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de Gacetas', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: flatGacetas.map((row) => cols.map((column) => formatDateField(row[column.dataKey]))), // EDITABLE
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Gacetas'); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Gacetas'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {gacetas ? gacetas.length : '0'}</span>
    ...
    const columnsData = [
        {
            nombrevar: "numero",
            header: "Número",
            filterPlaceholder: "Buscar por número de gaceta",
        },
        {
            nombrevar: "urlGaceta",
            header: "Url",
        ...
        return (
            <div className="flex align-items-center gap-2">
                <a
                    ...
                    href={rowData.urlGaceta}
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    <i className="pi pi-external-link" style={{ fontSize: '0.8rem', margin: '0' }}></i>
                </a>
            </div>
        );
    ...
    const onRowSelect = (event) => {
        props.onSelect(event.data.numero);
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="numero" // EDITABLE
                filters={filters}
                globalFilterFields={['numero', 'fecha', 'paisNombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron gacetas" // EDITABLE
                ...
                stateKey="gacetas-table-state" // EDITABLE
            >
                {columnsData.map((column, index) => (
                    column.nombrevar === 'urlGaceta' ? (
                        <Column
                            body={urlBodyTemplate}
                            key={index}
                            field={column.nombrevar}
                            header={column.header}
                            style={{ minWidth: '10px', maxWidth: '150px', width: '50px' }}
                        />
                    ) : (
                        <Column
                            key={index}
                            ...
                            style={{ minWidth: '10px', maxWidth: '200px', width: '100px' }}
                        />
                    )
                ))}
            </DataTable>
