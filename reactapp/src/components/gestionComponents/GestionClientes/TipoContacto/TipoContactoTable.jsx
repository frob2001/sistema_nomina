...
import { useTipoContactoClientes } from '../../../../services/useTipoContactoCliente'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function TipoContactoTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { tiposContactoClientes, error, isLoading, isValidating, refresh } = useTipoContactoClientes(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tipocontacto-table-state');
    ...
    const cols = [
        { header: 'Tipo de contacto', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de contactos', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposContactoClientes.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de contactos.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de contactos'); // EDITABLE
        ...
        tiposContactoClientes.forEach((tipoContacto) => {
            worksheet.addRow([
                tipoContacto.nombre,
            ]);
        }); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de contactos'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposContactoClientes ? tiposContactoClientes.length : '0'}</span>
    ...
    const onRowSelect = (event) => {
        props.onSelect(event.data.tipoContactoClienteId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                value={tiposContactoClientes} // EDITABLE
                resizableColumns
                removableSort
                paginator
                scrollable
                ...
                size="small"
                dataKey="tipoContactoClienteId" // EDITABLE
                filters={filters}
                globalFilterFields={['nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de contactos" // EDITABLE
                ...
                stateKey="tipocontacto-table-state" // EDITABLE
            >
...
export default TipoContactoTable;