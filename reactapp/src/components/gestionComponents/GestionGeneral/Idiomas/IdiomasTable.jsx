...
import { useIdiomas } from '../../../../services/useIdiomas';
import pdfHeaderVertical from '../../../../assets/CorralBannerVertical.png';

function IdiomasTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { idiomas, error, isLoading, isValidating, refresh } = useIdiomas(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('idiomas-table-state');
    ...
    const initFilters = () => {
        setFilters({
            ...
            codigoIdioma: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
            nombre: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        }); // EDITABLE
        setGlobalFilterValue('');
    }; // Función para restaurar e inicializar los filtros: ESPECIFICO
    const cols = [
        { header: 'Código Idioma', dataKey: 'codigoIdioma' },
        { header: 'Idioma', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de Idiomas', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: idiomas.map((row) => cols.map((column) => formatDateField(row[column.dataKey]))), // EDITABLE
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Idiomas'); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Idiomas'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {idiomas ? idiomas.length : '0'}</span>
    ...
    const columnsData = [
        {
            nombrevar: "codigoIdioma",
            header: "Código Idioma",
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="codigoIdioma" // EDITABLE
                filters={filters}
                globalFilterFields={['codigoIdioma', 'nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron idiomas" // EDITABLE
                metaKeySelection={false}
                stateStorage="session" 
                stateKey="idiomas-table-state" // EDITABLE
