...
import { useTiposEstados } from '../../../../services/useTiposEstados'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function TipoEstadoTable(props) {
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tipoestado-table-state'); // EDITAR
    ...
    const initFilters = () => {
        setFilters({
            ...
            tipoEstadoId: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
            nombreEspanol: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
            nombreIngles: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
        }); // EDITABLE
        setGlobalFilterValue('');
    }; // Función para restaurar e inicializar los filtros: EDITABLE
    const cols = [
        { header: 'Código', dataKey: 'tipoEstadoId' },
        { header: 'Nombre (ES)', dataKey: 'nombreEspanol' },
        { header: 'Nombre (EN)', dataKey: 'nombreIngles' }
    ]; // Columnas que se expotarán en el PDF: EDITABLE
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de estados', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposEstados.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de estados.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de estados'); // EDITABLE
        ...
        tiposEstados.forEach((tipoEstado) => {
            worksheet.addRow([
                tipoEstado.tipoEstadoId,
                tipoEstado.nombreEspanol,
                tipoEstado.nombreIngles,
            ]);
        }); // EDITABLE

        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de estados'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposEstados ? tiposEstados.length : '0'}</span>
    ...
    const columnsData = [
        {
            nombrevar: "tipoEstadoId",
            header: "Código",
            filterPlaceholder: "Buscar por código",
        },
        {
            nombrevar: "nombreEspanol",
            header: "Nombre (ES)",
            filterPlaceholder: "Buscar por nombre (ES)",
        },
        {
            nombrevar: "nombreIngles",
            header: "Nombre (EN)",
            filterPlaceholder: "Buscar por nombre (EN)",
        }
    ]; // Contiene los parámetros para crear columnas: EDITABLE
    const onRowSelect = (event) => {
        props.onSelect(event.data.tipoEstadoId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="tipoEstadoId" // EDITABLE
                filters={filters}
                globalFilterFields={['tipoEstadoId', 'nombreEspanol', 'nombreIngles']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de estados" // EDITABLE
                selectionMode={error ? null : "single"}
                selection={selectedObject}
                onSelectionChange={(e) => !error && setSelectedObject(e.value)} 
                onRowSelect={onRowSelect}
                onRowUnselect={onRowUnselect}
                metaKeySelection={false}
                stateStorage="session" 
                stateKey="tipoestado-table-state" // EDITABLE
            >
...
export default TipoEstadoTable;