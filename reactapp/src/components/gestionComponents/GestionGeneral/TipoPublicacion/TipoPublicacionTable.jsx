...
import { useTipoPublicacion } from '../../../../services/useTipoPublicacion'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function TipoPublicacionTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { tiposPublicaciones, error, isLoading, isValidating, refresh } = useTipoPublicacion(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tipopublicacion-table-state'); // EDITAR
    ...
    const cols = [
        { header: 'Tipo de publicación', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de publicaciones', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposPublicaciones.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de publicaciones.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de publicaciones'); // EDITABLE
        ...
        tiposPublicaciones.forEach((tipopublicacion) => {
            worksheet.addRow([
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de publicaciones'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposPublicaciones ? tiposPublicaciones.length : '0'}</span>
    ...
    const columnsData = [
        {
            ...
            filterPlaceholder: "Buscar por tipo de publicación",
        }
    ]; // Contiene los parámetros para crear columnas: EDITABLE
    const onRowSelect = (event) => {
        props.onSelect(event.data.tipoPublicacionId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                value={tiposPublicaciones} // EDITABLE
                resizableColumns
                removableSort
                paginator
                scrollable
                ...
                size="small"
                dataKey="tipoPublicacionId" // EDITABLE
                filters={filters}
                globalFilterFields={['nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de publicaciones" // EDITABLE
                ...
                stateKey="tipopublicacion-table-state" // EDITABLE
            >
...
export default TipoPublicacionTable;