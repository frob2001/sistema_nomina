...
import { useTipoReferencia } from '../../../../services/useTipoReferencia'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function TipoReferenciaTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { tiposReferencias, error, isLoading, isValidating, refresh } = useTipoReferencia(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tiporeferencia-table-state'); // EDITAR
    ...
    const cols = [
        { header: 'Tipo de referencia', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de referencias', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposReferencias.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de referencias.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de referencias'); // EDITABLE
        ...
        tiposReferencias.forEach((tiporeferencia) => {
            worksheet.addRow([
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de referencias'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposReferencias ? tiposReferencias.length : '0'}</span>
    ...
    const columnsData = [
        {
            ...
            filterPlaceholder: "Buscar por tipo de referencia",
        }
    ]; // Contiene los parámetros para crear columnas: EDITABLE
    const onRowSelect = (event) => {
        props.onSelect(event.data.tipoReferenciaId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                value={tiposReferencias} // EDITABLE
                resizableColumns
                removableSort
                paginator
                scrollable
                ...
                size="small"
                dataKey="tipoReferenciaId" // EDITABLE
                filters={filters}
                globalFilterFields={[ 'nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de referencias" // EDITABLE
                ...
                stateKey="tiporeferencia-table-state" // EDITABLE
            >
...
export default TipoReferenciaTable;