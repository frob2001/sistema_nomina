...
import { useAutoridad } from '../../../../services/useAutoridad'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function AutoridadTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { autoridades, error, isLoading, isValidating, refresh } = useAutoridad(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('autoridad-table-state');
    ...
    const cols = [
        { header: 'Autoridad', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de autoridades', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: autoridades.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Autoridades.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Autoridades'); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Autoridades'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {autoridades ? autoridades.length : '0'}</span>
    ...
    const columnsData = [
        {
            nombrevar: "nombre",
            header: "Autoridades",
            filterPlaceholder: "Buscar por nombre de la autoridad",
        }
    ]; // Contiene los parámetros para crear columnas: ESPECIFICO
    const onRowSelect = (event) => {
        props.onSelect(event.data.autoridadId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="autoridadId" // EDITABLE
                filters={filters}
                globalFilterFields={[ 'nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron autoridades" // EDITABLE
                ...
                stateKey="autoridad-table-state" // EDITABLE
            >
