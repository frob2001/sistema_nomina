...
import { useTipoAccion } from '../../../../services/useTipoAccion'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function TipoAccionTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { tiposAcciones, error, isLoading, isValidating, refresh } = useTipoAccion(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tipoaccion-table-state');
    ...
    const cols = [
        { header: 'Tipo de acción a tercero', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de acciones a terceros', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposAcciones.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de acciones a terceros.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de acciones a terceros'); // EDITABLE
        ...
        tiposAcciones.forEach((tipoAccion) => {
            worksheet.addRow([
                tipoAccion.nombre,
            ]);
        }); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de acciones a terceros'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposAcciones ? tiposAcciones.length : '0'}</span>
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="tipoAccionId" // EDITABLE
                filters={filters}
                globalFilterFields={[ 'nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de acciones a terceros" // EDITABLE
                ...
                stateKey="tipoaccion-table-state" // EDITABLE
            >
...
export default TipoAccionTable;