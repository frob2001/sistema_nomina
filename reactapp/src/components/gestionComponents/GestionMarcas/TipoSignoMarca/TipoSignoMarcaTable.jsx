...
import { useTipoSignoMarca } from '../../../../services/useTipoSignoMarca'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function TipoSignoMarcaTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { tiposSignoMarcas, error, isLoading, isValidating, refresh } = useTipoSignoMarca(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tiposignomarca-table-state');
    ...
    const cols = [
        { header: 'Tipo de signo de marca', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de signos', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposSignoMarcas.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de signos de marcas.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de signos de una marca'); // EDITABLE
        ...
        tiposSignoMarcas.forEach((tipoSignoMarca) => {
            worksheet.addRow([
                tipoSignoMarca.nombre,
            ]);
        }); // EDITABLE

        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de signos de marca'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposSignoMarcas ? tiposSignoMarcas.length : '0'}</span>
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                value={tiposSignoMarcas} // EDITABLE
                resizableColumns
                removableSort
                paginator
                scrollable
                ...
                size="small"
                dataKey="tipoSignoMarcaId" // EDITABLE
                filters={filters}
                globalFilterFields={['nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de signos marcas" // EDITABLE
                ...
                stateKey="tiposignomarca-table-state" // EDITABLE
            >
...
export default TipoSignoMarcaTable;