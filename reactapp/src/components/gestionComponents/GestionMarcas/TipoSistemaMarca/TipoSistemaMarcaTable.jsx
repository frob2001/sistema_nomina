...
import { useTipoSistemaMarca } from '../../../../services/useTipoSistemaMarca'; // EDITAR CON EL SERVICIO CORRESPONDIENTE

function TipoSistemaMarcaTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { tiposSistemaMarcas, error, isLoading, isValidating, refresh } = useTipoSistemaMarca(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tiposistemamarca-table-state');
    ...
    const cols = [
        { header: 'Tipo de sistema de marca', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de sistemas', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposSistemaMarcas.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de sistemas.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de sistemas de una marca'); // EDITABLE
        ...
        tiposSistemaMarcas.forEach((tipoSistemaMarca) => {
            worksheet.addRow([
                tipoSistemaMarca.nombre,
            ]);
        }); // EDITABLE

        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de sistemas de marca'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposSistemaMarcas ? tiposSistemaMarcas.length : '0'}</span>
    ...
    const columnsData = [
        {
            nombrevar: "nombre",
            header: "Tipo de sistema de marca",
            filterPlaceholder: "Buscar por tipo de acciones a terceros",
        }
    ]; // Contiene los parámetros para crear columnas: ESPECIFICO
    const onRowSelect = (event) => {
        props.onSelect(event.data.tipoSistemaMarcaId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                value={tiposSistemaMarcas} // EDITABLE
                resizableColumns
                removableSort
                paginator
                scrollable
                ...
                size="small"
                dataKey="tipoSistemaMarcaId" // EDITABLE
                filters={filters}
                globalFilterFields={['nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de Sistemas marcas" // EDITABLE
                ...
                stateKey="tiposistemamarca-table-state" // EDITABLE
            >
...
export default TipoSistemaMarcaTable;