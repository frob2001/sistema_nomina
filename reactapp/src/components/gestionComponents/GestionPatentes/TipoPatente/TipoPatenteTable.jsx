...
function TipoPatenteTable(props) {

    // --------------- Setup (Servicios, Contextos, Referencias) -----------------------------------

    const toast = useRef(null);
    const { tiposPatentes, error, isLoading, isValidating, refresh } = useTipoPatente(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('tipopatente-table-state');
    ...
    const cols = [
        { header: 'Tipo de patente', dataKey: 'nombre' }
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de tipos de patentes', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: tiposPatentes.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
                ...
                doc.save('Tipos de patentes.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Tipos de patentes'); // EDITABLE
        ...
        tiposPatentes.forEach((tipoPatente) => {
            worksheet.addRow([
                tipoPatente.nombre,
            ]);
        }); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Tipos de patentes'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {tiposPatentes ? tiposPatentes.length : '0'}</span>
    ...
    const columnsData = [
        {
            ...
            filterPlaceholder: "Buscar por tipo de patentes",
        }
    ]; // Contiene los parámetros para crear columnas: ESPECIFICO
    const onRowSelect = (event) => {
        props.onSelect(event.data.tipoPatenteId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="tipoPatenteId" // EDITABLE
                filters={filters}
                globalFilterFields={['nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron tipos de patentes" // EDITABLE
                ...
                stateKey="tipopatente-table-state" // EDITABLE
            >
...
export default TipoPatenteTable;