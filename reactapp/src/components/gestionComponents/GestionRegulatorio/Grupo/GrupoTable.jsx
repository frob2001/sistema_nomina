...
import { useGrupos } from '../../../../services/useGrupos'; // EDITAR CON EL SERVICIO CORRESPONDIENTE
...
function GruposTable(props) {
    ...
    const { grupos, error, isLoading, isValidating, refresh } = useGrupos(); // EDITABLE
    ...
    useEffect(() => {
        ...
        const savedState = sessionStorage.getItem('grupos-table-state');
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de grupos', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: grupos.map((row) => cols.map((column) => row[column.dataKey])), // EDITABLE
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Grupos'); // EDITABLE
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Grupos'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {grupos ? grupos.length : '0'}</span>
    ...
    const onRowSelect = (event) => {
        props.onSelect(event.data.grupoId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="grupoId" // EDITABLE
                filters={filters}
                globalFilterFields={['nombre']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron grupos" // EDITABLE
                ...
                stateKey="grupos-table-state" // EDITABLE
            >
