...
function InventoresSearch({ onClose, onSearch }) { //EDITABLE

    // --------------- Redux store settings -------------------------------------------------------

    const dispatch = useDispatch();
    const inventorSearchData = useSelector(state => state.inventor.InventorSearch); // EDITABLE
    ...
    useEffect(() => {
        if (inventorSearchData) { // EDITABLE
            setSearchParameters(inventorSearchData.searchParameters || null); // EDITABLE
            setSelectedPais(inventorSearchData.selectedPais || null);
        }
    }, []); // Se ejecuta cuando el componente renderiza, lee los datos guardados: GENERAL
    const saveState = () => {
        dispatch(saveData({ objectName: 'InventorSearch', value: { searchParameters, selectedPais } })); // EDITABLE
    ...
    }, [searchParameters, selectedPais]); // Se ejecuta con cada cambio de estado.
    const deletePersistedStates = () => {
        dispatch(deleteData({ objectName: 'InventorSearch' })); // EDITABLE
    ...
    }, [isValidatingP]); // Cambia el estado de refreshing de los DROPDOWNs
                        ...
                        <section>
                            ...
                            <div className="form-body-group">
                                <div className="form-group">
                                    <label>Código</label>
                                    <input type="number" name="inventorId" value={searchParameters?.inventorId || ''} onChange={handleInputChange} maxLength="8" placeholder="Código único del inventor"/>
                                </div>
                                <div className="form-group">
                                    <label>Nombre</label>
                                    <input type="text" name="nombre" value={searchParameters?.nombre || ''} onChange={handleInputChange} maxLength="70" placeholder="Nombre del inventor" />
                                </div>
                                <div className="form-group">
                                    <label>Apellido</label>
                                    <input type="text" name="apellido" value={searchParameters?.apellido || ''} onChange={handleInputChange} maxLength="70" placeholder="Apellido del inventor" />
                                </div>
                            </div>
                        </section>
                        <section>
                            ...
                            <div className="form-body-group">
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="text" name="direccion" value={searchParameters?.direccion || ''} onChange={handleInputChange} maxLength="200" placeholder="Dirección del inventor" />
                                </div>
                            </div>
                        </section>
                    </form>
...
export default InventoresSearch;