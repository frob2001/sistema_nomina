...
const API_BASE_URL = `${apiEndpoint}/Inventores/Buscar`;
// Auth
import { useMsal } from '@azure/msal-react';

function InventoresTable(props) {
    ...
    const { data: inventoresPre, error, isValidating, isLoading, mutate: refresh } = useSWR(searchURL, fetcher, {
        errorRetryInterval: 10000,
    });

    const inventores = useMemo(() => {
        if (inventoresPre) {
            return inventoresPre.map(inventor => ({
                ...inventor,
                paisNombre: inventor.pais?.nombre,
                nombreCompleto: inventor.nombre + ' ' + inventor.apellido,
            }));
        }
    }, [inventoresPre]); // Añade el nombre del país como propiedad para que funcione bien con la datatable
    ...
    useEffect(() => {
        const savedState = sessionStorage.getItem('inventores-table-state'); // EDITABLE
    ...
    const initFilters = () => {
        setFilters({
            ...
            inventorId: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
            nombreCompleto: { operator: FilterOperator.AND, constraints: [{ value: null, matchMode: FilterMatchMode.STARTS_WITH }] },
    ...
    const cols = [
        { header: 'Código del Inventor', dataKey: 'inventorId' },
    ...
    const exportPdf = () => {
        ...
        import('jspdf').then((jsPDF) => {
            import('jspdf-autotable').then(() => {
                ...
                doc.text('Listado de Inventores', margin, 160, { align: 'left' }); // Left aligned
                ...
                doc.autoTable({
                    ...
                    body: inventores.map((row) => cols.map((column) => formatDateField(row[column.dataKey]))), // EDITABLE
                ...
                doc.save('Inventores.pdf'); // EDITABLE
                setPdfLoading(false);
            });
        }).catch((error) => {
            toast.current.show({
    ...
    const exportExcel = () => {
        ...
        const worksheet = workbook.addWorksheet('Inventores'); // EDITABLE
        ...
        inventores.forEach((inv) => { // EDITABLE
            worksheet.addRow([ 
                inv.inventorId,
                inv.nombre,
                inv.apellido,
        ...
        workbook.xlsx.writeBuffer().then((buffer) => {
            saveAsExcelFile(buffer, 'Inventores'); // EDITABLE
    ...
    const renderHeader = () => {
        return (
            <div className="flex justify-content-between">
                ...
                <div className="table-utility">
                    <span style={{ alignSelf: 'center', fontWeight: '400' }}><strong>Registros:</strong> {inventores ? inventores.length : '0'}</span>
    ...
    const columnsData = [
        {
            nombrevar: "inventorId",
            header: "Código",
            filterPlaceholder: "Buscar por código",
        },
        {
            nombrevar: "nombreCompleto",
    ...
    const onRowSelect = (event) => {
        props.onSelect(event.data.inventorId); // EDITABLE
    ...
    return (
        <div className="tabla">
            ...
            <DataTable
                ...
                size="small"
                dataKey="inventorId" // EDITABLE
                filters={filters}
                globalFilterFields={['inventorId', 'nombreCompleto', 'paisNombre', 'direccion']} // EDITABLE
                header={header}
                emptyMessage="No se encontraron inventores" // EDITABLE
                ...
                stateKey="inventores-table-state" // EDITABLE
            >
...
export default InventoresTable;