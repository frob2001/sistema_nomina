...
import { saveData } from '../../../context/patenteSlice';// EDITABLE

// Components
import EmptyTable from '../../administracionComponents/Empty/EmptyTable';
import PatentesSearch from './PatentesSearch';
import PatentesTable from './PatentesTable';
import PatentesCreate from './PatentesCreate';
import PatentesDetails from './PatentesDetails';
...
function PatentesPage() {
    ...
    const patentesPageData = useSelector(state => state.patente.PatentePage); // EDITABLE: persistencia de este componente
    ...
    useEffect(() => {
        if (patentesPageData) {
            setIsCreating(patentesPageData.isCreating || false);
            setIsSearching(patentesPageData.isSearching || false);
            setIsViewing(patentesPageData.isViewing || false);
            setIdSelected(patentesPageData.idSelected || null);
            setSearchUrl(patentesPageData.searchUrl || "");
            setWasSearched(patentesPageData.wasSearched || false);
        }
    }, []); // Se ejecuta cuando el componente renderiza, lee los datos guardados: ESPECIFICO
    const saveState = () => {
        dispatch(saveData({ objectName: 'PatentePage', value: { isCreating, isSearching, isViewing, idSelected, searchUrl, wasSearched } }));
    ...
    return (
        <>
            ...
            <div className="page-container">
                <h5 className="page-title">Patentes</h5>
                ...
                <div className="page-table">
                    {wasSearched ? <PatentesTable url={searchUrl} onClose={() => setIsViewing(false)} onSelect={(patenteId) => { setIsViewing(true); setIdSelected(patenteId); }} refreshTrigger={refreshTable} /> : <EmptyTable title='Patentes'/>}
                </div>
            </div>
            {isSearching && <PatentesSearch onClose={() => setIsSearching(false)} onSearch={(search) => { setSearchUrl(search); setWasSearched(true) }} />}
            {isCreating && <PatentesCreate onCreated={onCreated} onClose={() => setIsCreating(false)} />}
