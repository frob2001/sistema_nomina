...
const useAbogados = () => {
    ...
    const createAbogado = async (abogadoData) => {
        try {
            const accessToken = await getAccessToken();
            ...
            throw new Error("Hubo un error al ingresar el abogado");
        }
    };

    const updateAbogado = async (abogadoId, updatedData) => {
        try {
            const accessToken = await getAccessToken();
            const response = await fetch(`${API_BASE_URL}/${abogadoId}`, {
                method: 'PUT',
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                    'Content-Type': 'application/json',
            ...
            throw new Error("Hubo un error al editar el abogado");
        }
    };


    const deleteAbogado = async (abogadoId) => {
        try {
            const accessToken = await getAccessToken();
            const response = await fetch(`${API_BASE_URL}/${abogadoId}`, {
                method: 'DELETE',
            ...
            throw new Error("Hubo un error al eliminar el abogado");
        }
    };
